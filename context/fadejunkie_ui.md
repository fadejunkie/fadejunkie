...
- **Safe pattern:** `style={{ fontFamily: "var(--font-mono), ui-monospace, monospace" }}` — use `--font-mono` in all component inline style `fontFamily` values.
- **Detection:** Grep `font-geist-mono` in all `.tsx` files. Every hit is a P1 — should be `--font-mono`.
