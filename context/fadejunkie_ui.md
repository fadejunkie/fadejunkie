...
- **borderRadius specifically:** `style={{ borderRadius: "var(--radius-3xl)" }}` → `rounded-3xl`. `style={{ borderRadius: "var(--radius-xl)" }}` → `rounded-xl`. `style={{ borderRadius: "0.75rem" }}` → `rounded-xl`. Never use hardcoded pixel values like `"20px"` — always map to the nearest design token class (`rounded-2xl` = 1rem ≈ 16px, `rounded-3xl` = 1.5rem = 24px).
- **Conflict pattern:** Some components pass `borderRadius` via inline style AND have a `rounded-*` className simultaneously. The inline style WINS the cascade (always), making the Tailwind class dead code. These double-violations are especially confusing for future readers — the className appears to set the radius but doesn't. Fix: remove the style prop entirely, keep only the Tailwind class.
  ...
  + Also noted: `app/(auth)/profile/page.tsx:101`, `components/PathSelector.tsx:244`, `components/PostComposer.tsx:31` — each has borderRadius inline style violations (some with conflicting Tailwind class). Queued for future cycles.
