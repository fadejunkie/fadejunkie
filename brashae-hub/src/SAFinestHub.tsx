...
const PAYMENT_URL = "https://square.link/u/zb4zYJlU";
...
/* ═══════════════════════════════════════
   LOGO GALLERY + LIGHTBOX
   ═══════════════════════════════════════ */

const LOGO_CATEGORIES = [
  { slug: "bold-block",      label: "BOLD BLOCK" },
  { slug: "condensed-sans",  label: "CONDENSED SANS" },
  { slug: "editorial-serif", label: "EDITORIAL SERIF" },
  { slug: "brush-script",    label: "BRUSH SCRIPT" },
  { slug: "script-serif",    label: "SCRIPT SERIF" },
];

function detectCategory(label: string): string {
  const l = label.toLowerCase();
  if (l.includes("bold") && l.includes("block"))      return "bold-block";
  if (l.includes("condensed"))                          return "condensed-sans";
  if (l.includes("editorial") || (l.includes("serif") && !l.includes("script"))) return "editorial-serif";
  if (l.includes("brush"))                              return "brush-script";
  if (l.includes("script"))                             return "script-serif";
  return "other";
}
...
function LogoLightbox({ images, startIndex, onClose }: {
  images: { url: string; label: string; categoryLabel: string }[];
  startIndex: number;
  onClose: () => void;
}) {
  ...
  return (
    ...
    >
      {/* Header bar */}
      <div onClick={e => e.stopPropagation()} style={{ position: "absolute", top: 0, left: 0, right: 0, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px" }}>
        <div>
          <div style={{ fontSize: 9, color: GOLD, letterSpacing: 3, fontFamily: "'IBM Plex Mono',monospace" }}>{current.categoryLabel}</div>
          <div style={{ fontSize: 13, color: "#f5f0e8", fontWeight: 600, marginTop: 2 }}>{current.label}</div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ fontSize: 11, color: "#666", fontFamily: "'IBM Plex Mono',monospace" }}>{idx + 1} / {images.length}</div>
          <button onClick={onClose} style={{ background: "none", border: "none", color: "#888", fontSize: 24, cursor: "pointer", lineHeight: 1, padding: "4px 8px" }}>×</button>
        </div>
      </div>

      {/* Image */}
      <div onClick={e => e.stopPropagation()} style={{ display: "flex", alignItems: "center", justifyContent: "center", width: "100%", padding: "80px 80px 60px" }}>
        <img
          src={current.url}
          alt={current.label}
          style={{ maxWidth: "90vw", maxHeight: "75vh", objectFit: "contain", borderRadius: 4, background: "#fff", padding: "24px 32px" }}
        />
      </div>
      ...
      {canPrev && (
        <button onClick={e => { e.stopPropagation(); prev(); }}
          style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", background: "rgba(255,255,255,0.08)", border: "none", borderRadius: 8, width: 48, height: 64, cursor: "pointer", color: "#fff", fontSize: 22, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ‹
        </button>
      )}
      {canNext && (
        <button onClick={e => { e.stopPropagation(); next(); }}
          style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "rgba(255,255,255,0.08)", border: "none", borderRadius: 8, width: 48, height: 64, cursor: "pointer", color: "#fff", fontSize: 22, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ›
        </button>
      )}

      {/* Bottom strip — thumbnail dots */}
      <div onClick={e => e.stopPropagation()} style={{ position: "absolute", bottom: 16, display: "flex", gap: 5, alignItems: "center" }}>
        {images.map((_, i) => (
          <button key={i} onClick={() => setIdx(i)}
            style={{ width: i === idx ? 20 : 6, height: 6, borderRadius: 3, background: i === idx ? GOLD : "rgba(255,255,255,0.25)", border: "none", cursor: "pointer", padding: 0, transition: "all 0.2s" }} />
        ))}
      </div>
    </div>
  );
...
function LogoGallery({ deliverables, isOps, onRemove }: {
  deliverables: any[];
  isOps: boolean;
  ...
  // Split: image deliverables vs file/link deliverables
  const imageItems = deliverables.filter(d => isImageUrl(d.url));
  const otherItems = deliverables.filter(d => !isImageUrl(d.url));

  // Build flat gallery list ordered by category
  const flatGallery: { url: string; label: string; categoryLabel: string; id: string }[] = [];
  LOGO_CATEGORIES.forEach(cat => {
    imageItems
      .filter(d => detectCategory(d.label) === cat.slug)
      .forEach(d => flatGallery.push({ url: d.url, label: d.label, categoryLabel: cat.label, id: d._id }));
  });
  // Uncategorized images
  imageItems
    .filter(d => detectCategory(d.label) === "other")
    .forEach(d => flatGallery.push({ url: d.url, label: d.label, categoryLabel: "OTHER", id: d._id }));

  // Group for display
  const groupedCategories = LOGO_CATEGORIES.map(cat => ({
    ...cat,
    items: imageItems.filter(d => detectCategory(d.label) === cat.slug),
  })).filter(g => g.items.length > 0);
  const uncategorized = imageItems.filter(d => detectCategory(d.label) === "other");

  return (
    <>
      {/* Logo gallery — by category */}
      {groupedCategories.map(group => (
        <div key={group.slug} style={{ marginBottom: 28 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            <div style={{ fontSize: 9, fontWeight: 700, color: GOLD, letterSpacing: 3, fontFamily: "'IBM Plex Mono',monospace" }}>{group.label}</div>
            <div style={{ flex: 1, height: 1, background: t.BORDER }} />
            <div style={{ fontSize: 9, color: t.MUTED, fontFamily: "'IBM Plex Mono',monospace" }}>{group.items.length}</div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 10 }}>
            {group.items.map(d => {
              const flatIdx = flatGallery.findIndex(g => g.id === d._id);
              return (
                <div key={d._id} style={{ position: "relative" }}>
                  <div onClick={() => setLightboxIndex(flatIdx)}
                    style={{ background: t.mode === "dark" ? "#1a1a1a" : "#fff", border: `1px solid ${t.BORDER}`, borderRadius: 8, overflow: "hidden", cursor: "pointer", aspectRatio: "4/3", display: "flex", alignItems: "center", justifyContent: "center", padding: 12, transition: "border-color 0.15s, box-shadow 0.15s" }}
                    onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = GOLD; (e.currentTarget as HTMLElement).style.boxShadow = `0 0 0 1px ${GOLD}`; }}
                  ...
                  >
                    <img src={d.url} alt={d.label} style={{ width: "100%", height: "100%", objectFit: "contain" }} />
                  </div>
                  <div style={{ fontSize: 10, color: t.DIM, marginTop: 5, paddingLeft: 2, fontFamily: "'IBM Plex Mono',monospace", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{d.label}</div>
                  {isOps && (
                    <button onClick={() => onRemove(d._id)} style={{ position: "absolute", top: 6, right: 6, width: 20, height: 20, borderRadius: "50%", background: "rgba(0,0,0,0.5)", color: "#fff", border: "none", cursor: "pointer", fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center", lineHeight: 1, padding: 0 }}>×</button>
                  )}
                </div>
              );
            })}
      ...
      {uncategorized.length > 0 && (
        <div style={{ marginBottom: 28 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            <div style={{ fontSize: 9, fontWeight: 700, color: t.DIM, letterSpacing: 3, fontFamily: "'IBM Plex Mono',monospace" }}>OTHER</div>
            <div style={{ flex: 1, height: 1, background: t.BORDER }} />
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 10 }}>
            {uncategorized.map(d => {
              ...
              return (
                <div key={d._id} style={{ position: "relative" }}>
                  <div onClick={() => setLightboxIndex(flatIdx)}
                    style={{ background: t.mode === "dark" ? "#1a1a1a" : "#fff", border: `1px solid ${t.BORDER}`, borderRadius: 8, overflow: "hidden", cursor: "pointer", aspectRatio: "4/3", display: "flex", alignItems: "center", justifyContent: "center", padding: 12 }}>
                    <img src={d.url} alt={d.label} style={{ width: "100%", height: "100%", objectFit: "contain" }} />
                  </div>
                  <div style={{ fontSize: 10, color: t.DIM, marginTop: 5, fontFamily: "'IBM Plex Mono',monospace" }}>{d.label}</div>
                  {isOps && (
                    <button onClick={() => onRemove(d._id)} style={{ position: "absolute", top: 6, right: 6, width: 20, height: 20, borderRadius: "50%", background: "rgba(0,0,0,0.5)", color: "#fff", border: "none", cursor: "pointer", fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center", lineHeight: 1, padding: 0 }}>×</button>
                  )}
                </div>
      ...
      {/* Non-image deliverables — keep as list */}
      {otherItems.length > 0 && (
        <div>
          <SectionHeader label="OTHER FILES" />
          {otherItems.map((d: any) => (
            <div key={d._id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", background: t.CARD, border: `1px solid ${t.BORDER}`, borderRadius: 8, marginBottom: 8 }}>
              ...
              {isOps && <button onClick={() => onRemove(d._id)} style={{ background: "none", border: "none", color: t.DIM, cursor: "pointer", fontSize: 18, lineHeight: 1, flexShrink: 0 }}>×</button>}
            </div>
          ))}
        </div>
      )}
      ...
      {/* Lightbox */}
      {lightboxIndex !== null && flatGallery.length > 0 && (
        <LogoLightbox images={flatGallery} startIndex={lightboxIndex} onClose={() => setLightboxIndex(null)} />
      )}
    </>
  );
}
...
/* ═══════════════════════════════════════
   DELIVERABLES PAGE — CONCEPTS + FINAL
   ═══════════════════════════════════════ */
function DeliverablesPage({ isOps }: { isOps: boolean }) {
  ...
  // Ops form state
  const [section, setSection] = useState<"concepts"|"finals">("concepts");
  const [label,   setLabel]   = useState("");
  const [url,     setUrl]     = useState("");
  const [adding,  setAdding]  = useState(false);

  // Split by milestoneKey
  const concepts = (deliverables ?? []).filter(d => d.milestoneKey === "logo-design" || d.milestoneKey === "p1" || !d.milestoneKey);
  const finals   = (deliverables ?? []).filter(d => d.milestoneKey === "logo-finals");
  ...
  const handleAdd = async () => {
    ...
    await addDeliverable({
      clientSlug: CLIENT_SLUG,
      projectId: PROJECT_ID,
      milestoneKey: section === "finals" ? "logo-finals" : "logo-design",
      label: label.trim(),
  ...
  const downloadReady = PAYMENT_URL.length > 0;

  // Still loading from Convex
  if (deliverables === undefined) {
    return (
      <div style={{ padding: "60px 16px", textAlign: "center", color: t.DIM, fontSize: 13 }}>
        Loading...
      </div>
    );
  }
  ...
  return (
    <div style={{ padding: "20px 16px 48px" }}>
      ...
      {/* ── CONCEPTS — only show when ops or has content ── */}
      {(isOps || concepts.length > 0) && (
        <>
          <SectionHeader label="CONCEPTS" />
          <div style={{ fontSize: 13, color: t.DIM, marginBottom: 20, lineHeight: 1.6 }}>
            {isOps ? "V1 explorations delivered to Jay." : "Tap any concept to view full size. Swipe or use arrows to move through styles."}
          </div>
          {concepts.length === 0 ? (
            <div style={{ textAlign: "center", padding: "32px 0", color: t.DIM, fontSize: 13 }}>No concepts added yet.</div>
          ) : (
            <LogoGallery deliverables={concepts} isOps={isOps} onRemove={id => removeDeliverable({ id })} />
          )}
        </>
      )}
      ...
      <div style={{ marginTop: 36 }}>
        ...
        <div style={{ fontSize: 13, color: t.DIM, marginBottom: 20, lineHeight: 1.6 }}>
          {isOps ? "Approved finals — watermarked previews before payment." : "Your approved logo package. Purchase to unlock all files."}
        </div>
        ...
        {finals.length > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: 12, marginBottom: 28 }}>
            {finals.map(d => (
              <div key={d._id} style={{ position: "relative" }}>
                <div style={{ background: t.mode === "dark" ? "#1a1a1a" : "#fff", border: `1px solid ${t.BORDER}`, borderRadius: 10, padding: 16, display: "flex", alignItems: "center", justifyContent: "center", aspectRatio: "1/1" }}>
                  {isImageUrl(d.url) ? (
                    <img src={d.url} alt={d.label} style={{ width: "100%", height: "100%", objectFit: "contain" }} />
                  ) : (
                    <div style={{ fontSize: 11, color: t.DIM, textAlign: "center" }}>{d.label}</div>
                  )}
                </div>
                <div style={{ fontSize: 10, color: t.DIM, marginTop: 6, fontFamily: "'IBM Plex Mono',monospace", textAlign: "center", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{d.label}</div>
                {isOps && (
                  <button onClick={() => removeDeliverable({ id: d._id })}
                    style={{ position: "absolute", top: 6, right: 6, width: 20, height: 20, borderRadius: "50%", background: "rgba(0,0,0,0.5)", color: "#fff", border: "none", cursor: "pointer", fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center", lineHeight: 1, padding: 0 }}>×</button>
                )}
              </div>
            ))}
          </div>
        ...
        {finals.length === 0 && isOps && (
          <div style={{ textAlign: "center", padding: "24px 0", color: t.DIM, fontSize: 13, marginBottom: 24 }}>No finals added yet — use the form below.</div>
        )}

        {/* Download CTA */}
        <div style={{ background: t.CARD, border: `1px solid ${downloadReady ? GOLD : t.BORDER}`, borderRadius: 10, padding: "20px 20px 24px", marginTop: finals.length > 0 ? 0 : 8 }}>
          <button
            onClick={() => downloadReady && window.open(PAYMENT_URL, "_blank")}
            style={{
              width: "100%", padding: "16px",
              background: downloadReady ? GOLD : t.BORDER,
              color: downloadReady ? "#1a1410" : t.SUBTLE,
            ...
            {downloadReady ? "DOWNLOAD NOW — $250" : "PAYMENT LINK COMING SOON"}
          </button>
          <div style={{ fontSize: 11, color: t.MUTED, fontFamily: "'IBM Plex Mono',monospace", textAlign: "center", lineHeight: 1.6 }}>
            {downloadReady
              ? "Secure payment via Square · All 6 logo variants · SVG + PNG"
              : "Full package will be available here once finalized."}
          </div>
        </div>
      </div>
      ...
      {isOps && (
        <div style={{ background: t.CARD, border: `1px solid ${t.BORDER}`, borderRadius: 8, padding: "16px", marginTop: 32 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
            {(["concepts", "finals"] as const).map(s => (
              <button key={s} onClick={() => setSection(s)}
                style={{ flex: 1, padding: "8px 0", background: section === s ? GOLD : t.INPUT, color: section === s ? "#1a1410" : t.SUBTLE, border: `1px solid ${section === s ? GOLD : t.BORDER}`, borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: "pointer", letterSpacing: 2, fontFamily: "'IBM Plex Mono',monospace" }}>
                {s.toUpperCase()}
              </button>
            ))}
          </div>
          ...
          <input value={label} onChange={e => setLabel(e.target.value)}
            placeholder={section === "finals" ? "Label (e.g. Black — Watermark)" : "Label (e.g. Bold Block — V1)"}
          ...
          <button onClick={handleAdd} disabled={adding || !label.trim()}
            ...
            {adding ? "ADDING..." : `ADD TO ${section.toUpperCase()}`}
          </button>
        </div>
      )}
    </div>
