...
export function Navbar({ scrollReveal = false }: NavbarProps) {
  const [sessionId, setSessionId] = useState("");
  const [menuOpen, setMenuOpen] = useState(false);
  const [navOpacity, setNavOpacity] = useState(scrollReveal ? 0 : 1);
  ...
  useEffect(() => {
    ...
    const handler = (e: Event) => {
      ...
      // Fade in from 85% → 100% of video scroll progress
      const op = Math.max(0, Math.min(1, (p - 0.85) / 0.15));
      setNavOpacity(op);
    };
    window.addEventListener("scroll-video-progress", handler);
    return () => window.removeEventListener("scroll-video-progress", handler);
  }, [scrollReveal]);
