...
export const setTask = mutation({
  ...
  handler: async (ctx, { projectId, key, value }) => {
    ...
    if (existing) {
      ...
      await ctx.db.insert("chucoTasks", { projectId, taskKey: key, completed: value });
    }
  },
});
...
export const saveDiscovery = mutation({
  ...
  handler: async (ctx, { projectId, responses }) => {
    ...
    if (existing) {
      ...
      await ctx.db.insert("chucoDiscovery", { projectId, responses, submittedAt: now });
    }
    await ctx.db.insert("chucoSubmissions", {
      projectId,
      type: "discovery",
      payload: responses,
      submittedAt: now,
...
export const saveAgreement = mutation({
  ...
  handler: async (ctx, args) => {
    const now = Date.now();
    await ctx.db.insert("chucoAgreements", {
      ...args,
      signedAt: now,
